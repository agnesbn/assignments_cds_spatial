...
This chunk is based on the work I did last week and so I will not be explaining it in that much detail. I basically just load in the `cities` and `inscriptions` objects and make sure that they have the same CRS. Then, I create a convex hull around the `inscriptions` points and select the cities that fall within.
```{r, eval=FALSE}
library(sf)
library(raster)
library(tidyverse)
...
# loading in the `cities` object, converting it into an sf object, and transforming its CRS to 3035
...
# loading in the inscriptions.json and converting it into a tibble
list_json <- jsonlite::fromJSON("../Week03/inscriptions.json")
inscriptions = as_tibble(list_json)

# getting the coordinates from the tibble, creating an sf object from those coordinates, and transforming the CRS to match the `cities` object
i <- inscriptions %>% 
...
# checking that the two objects have the same CRS - output = TRUE
st_crs(cities_sf3035) == st_crs(insc_sf3035)

# creating a convex hull around the inscriptions's points dissolved into a MULTIPOINT
insc_ch <- st_convex_hull(st_union(insc_sf3035))

# selecting those cities that fall within the convex hull of inscriptions
...
```{r}
summary(st_nearest_feature(cities_it,insc_sf3035))
```
...
*1. What is the average distance between the inscriptions and nearest city within the convex hull?* - If I did this correct, the mean distance between the inscriptions and nearest city within the convex hull is **5562.142 meters** or 5.562 kilometers.

*2. What is the problem with calculating distances between vector objects with st_distance()?* - I'm not quite sure... 

*3. What can you say about the spatial distribution of ancient inscriptions vis-a-vis the cities or Viking monuments in Aarhus municipality? What factors might be impacting the locations?* - Also, not quite sure, but I'm interested to hear the answer...
...
my_token <- "pk.eyJ1IjoiYWduZXNibiIsImEiOiJja3p5N3hsa3EwOTZwMnZvMzk5YnVhc2l2In0.EyrJLliETCH_mCZqL7XJwQ"
...
I decided to make a Mapbox basemap focused on my **adress**:
```{r}
my_address <- mb_geocode("Bernhardt Jensens Boulevard 95, Aarhus, 8000, Denmark ")
```
...
#### Creating an isochrone map around my address
```{r}
home_iso_drive <- mb_isochrone(my_address,
                               profile = "driving",
                               time = c(4, 8, 12))
...
leaflet() %>%
  ...
  addLegend(labels = c(4, 8, 12),
            colors = colors,
            title = "Drive-time<br/>around my appartment")
```
**Look at that! As I live on Aarhus Ø, the area you can reach within 4 minutes is much smaller than what we saw for Nobelparken.**
#### Creating map over the walking route between my adress and Nobelparken
```{r}
route_uni <- mb_directions(origin = my_address,
                           destination = nobel,
                           profile = "walking")

mapbox_map %>%
  addPolylines(data = route_uni, 
               popup = paste0(
                 "Distance (km): ",
                 round(route_uni$distance, 1), 
                 "<br/>Time (minutes): ",
                 round(route_uni$duration, 1)
               ))
```
**Aaaaand here you see my walking route to the university.**

How many and which Viking monuments are reachable within each of the three bands?

The number of monuments in each band is:

* 4-min band: 69
* 8-min band: 37
* 12-min band: 32

For the specific monument names, see the solution below.

```{r}
# import the Viking monuments and municipalities data
...
# filter out other time periods than the Viking age
viking <- monuments %>%
  dplyr::filter(datering == "Vikingetid")

# join the viking data with the municipalities data to select only the data points from Aarhus
...
```{r}
# create the seperate isochrones for the 4-, 8-, and 12-min bands
home_iso_drive4 <- mb_isochrone(my_address,
                                profile = "driving",
                                time = 4)

home_iso_drive8 <- mb_isochrone(my_address,
                                profile = "driving",
                                time = 8)

home_iso_drive12 <- mb_isochrone(my_address,
                                 profile = "driving",
                                 time = 12)


# identify the monuments in the different isochrones
dif4 <- st_difference(viking_aarhus, home_iso_drive4)
dif8 <- st_difference(viking_aarhus, home_iso_drive8)
dif12 <- st_difference(viking_aarhus,home_iso_drive12)

# count points in every group
nrow(dif4)
dif4$stednavn
...
We’ll first generate the isochrones using a vector of times, 1 through 45 at 1-minute intervals, around the Skejby hospital emergency room address. Next, we can visualize our overlapping isochrones. We’ll use the `viridis` color palette as we did previously in the tutorial, and generate a color palette derived from the `time` column in our dataset. Once specified, we can add these polygons to our Mapbox basemap with a mostly-transparent fill opacity.
```{r map-Skejby-isochrones, eval =FALSE}
library(mapboxapi)
...
Sys.setenv(OPENCAGE_KEY = "3b8de3d9e59242cfbf1a189211f8c7c1")
...
* Bike: 12,479,881,260 m^2 or 12,479.88126 km^2 of Midtjylland cannot be reached within a 20-min bikeride.
* Car: 5,794,256,011 m^2 or 5,794.256011 km^2 of Midtjylland cannot be reached within a 20-min drive.

**And here is how I came to that conclusion:**

I first load in the data on Danish regions, transform it into a sf object, and then select only the region "Midtjylland":
```{r}
regions <- getData("GADM", country = "DNK", level = 1)
region_tf <- st_transform(st_as_sf(regions))
midtjylland <- region_tf %>% 
  dplyr::filter(NAME_1 == "Midtjylland")
```

Then I use `st_difference()` and `st_area()` to calculate the area which cannot be reached by bike in 20 minutes or less:
```{r}
# for some reason these objects have to be defined again for knitting to work
h <- oc_forward_df(placename = hospitals, countrycode = "DK", limit =1, no_annotations = TRUE)
h_sf <- st_as_sf(h, coords = c("oc_lng", "oc_lat"), crs = 4326)
cycling_isos <- mb_isochrone(
  h_sf,
...
# first unionise the cycling polygons
cycling_un <- st_union(cycling_isos)
difference <- st_difference(midtjylland, cycling_un)

#project
difference_st <- st_transform(difference, crs = 25832)

#union
difference_un <- st_union(difference_st)

#st_area
st_area(difference_un)
```

We can plot the results in a map, where the large blue area is what is not covered by a 20-min bikeride and the red areas can be reached within a 20-min bikeride.
```{r}
mapbox_map %>%
  addPolygons(data = difference,
              popup = ~id) %>% 
  addPolygons(data=cycling_isos, color = "red") %>%
  addPolygons(data=midtjylland)
```


The same method as above can be applied to calculate the areas that can and cannot be reached within a 20-min drive.
```{r}
# again, I had to define this again in order to knit to HTML
driving_isos <- mb_isochrone(
  h_sf,
  profile = "driving",
  time = 20,
...
driving_un <- st_union(driving_isos)
difference2 <- st_difference(midtjylland, driving_un)


#project
difference2_st <- st_transform(difference2, crs = 25832)

#union
difference2_un <- st_union(difference2_st)

#st_area
st_area(difference2_un)
```

Again, we can illustrate the results in a map. Here, the yellow area can be reached within 20 minutes of driving, while the blue area cannot.
```{r}
mapbox_map %>%
  addPolygons(data = difference2,
              popup = ~id) %>%
  addPolygons(data=driving_isos, color = "yellow") %>% 
  addPolygons(data=midtjylland)
```
As you can see, the blue area is much smaller, but there are still rather large gaps in the maps where a hospital cannot be reached within a 20-min drive.

*2. How much do you need to increase the driving range so that everyone in Midtjylland can access a hospital (let's assume everyone has a car and just want to inform them of how long it will take)?* - As the maximum time supported in the `mb_isochrone` is 60 minutes and as **increasing the driving range to 60 minutes did not cover the whole region**, some citizens have more than an hour's drive to the nearest hospital. However, the remaining area was only 169,581819 km^2 and it seemed like at least some of these areas consist of national parks or other possibly uninhabitet areas.

**This is how I came to the conclusion:**

First, I try adjusting the driving range that was set to 20 minutes previously.
```{r}
library(opencage)

# Driving range
driving_new <- mb_isochrone(
  h_sf,
  profile = "driving",
  time = 60,
  id = "placename"
...
```{r}
all_un <- st_union(driving_new)
dif_all <- st_difference(midtjylland, all_un)

#project
dif_all_st <- st_transform(dif_all, crs = 25832)

#union
dif_all_un <- st_union(dif_all_st)
...
```{r}
mapbox_map %>%
  addPolygons(data=driving_new, color = "yellow") %>% 
  addPolygons(data=midtjylland, color = "transparant")
```
As you can see, almost all of Midtjylland and also a lot of the neighbouring regions are covered by a 60-min drive.
...
no_cars <- read_csv(as_data_frame("../data/no_cars.csv", header = FALSE))
names(no_cars) <- c("description","municipality","nc2020", "nc2021")
```
...
#read.csv(https://www.dst.dk/en/Statistik/emner/borgere/husstande-familier-og-boern/husstande-og-familier)
```
